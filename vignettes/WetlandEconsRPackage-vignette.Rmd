...
npv_parameters <- npv_parameters %>%
  mutate(
    npv = unlist(pmap(list(np1= canola_profit, np2= spwheat_profit, np3= flax_profit, np4= barley_profit, np5= fallow, 
                           wl= wl, dc=dc, r=0.87, t=50),npv_5yrotation))
  ) %>%
  select(wl, npv)

datatable(npv_parameters)
